...
public class CommonExceptionAdvice {
    ...
    public String exceptCommon(Exception exception){
        ...
        // 여기 내용으로 추가로, 트레이스, 추적, 왜 예외가 발생했는지? 어디서부터 발생했는지의 내용? 같이 추가해보기.
        // 병렬처리, 1) 전체 예외를 가지고 오고, 2) 버퍼 문자열에 하나씩 추가하고, 3) ul 태그를 닫아서 전달.
        Arrays.stream(exception.getStackTrace()).forEach(stackTraceElement -> {
            buffer.append("<li>" + stackTraceElement.toString() + "</li>");
        });
        buffer.append("</ul>");
        String result = buffer.toString(); // 전체 예외 발생한 추적 결과가 담겨져 있음
        return result;
    }
