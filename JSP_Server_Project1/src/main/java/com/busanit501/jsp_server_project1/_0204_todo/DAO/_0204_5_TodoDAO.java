...
public class _0204_5_TodoDAO {
    ...
    // try ~ catch 구문 대신, 어노테이션 이용해서 @Cleanup
    ...
    public List<_0204_2_TodoVO> selectAll() throws Exception {
        ...
        // 데이터베이스로 받아온 내용을 리스트로 변환하는 작업
        // 임시로 담아둘 리스트를 선언
        List<_0204_2_TodoVO> list = new ArrayList<>();

        // 반복문을 이용해서 데이터베이스의 내용을 -> 리스트 요소의 객체에 각각 담기 놀이
        // resultSet, 테이블, 준비는 0행부터 준비하고, next() 실행했을때, 데이터가 있으면 다음행으로 간다.
        // 예시) 데이터베이스의 현재 테이터의 테이블
        // 1행 : 1,샘플2수정,2026-02-03,0
        // 2행 : 3,샘플제목,2026-02-03,0
        // 3행 : 4,샘플제목22,2026-02-03,0

        while (resultSet.next()) {
            // 객체에 담기 위해서 임시 객체를 생성, builder 패턴 이용함
            ...
            // 임시 리스트에 데이터베이스의 객체 내용 -> 객체로 변경한 요소를
            // 리스트에 담기
            list.add(vo);
        }
        return list;
    ...
    // 한 개만 조회, 특징 : 조회할 tno 번호를 알고 있다고 가정.
    // 조회할 todo를 클릭하면, 클릭한 todo tno 번호를 화면으로부터 전달 받음.
    ...
    // 수정하기
    // 글 등록하기와 구조가 비슷함
    // 수정할 내용을 담을 데이터가 필요. 데이터 받아왔다고 가정하고 진행.
    // 나중에 화면에서 수정할 내용을 받아오게 됨. 받아오면 그 데이터를 여기 : _0203_1_TodoVO todoVO 담기
    // 여기서 _0203_1_TodoVO todoVO에서 변경할 내용을 꺼내서 데이터베이스에 수정.
    ...
    public void deleteOne(Long tno) throws Exception {
        // tno = 4 삭제하기
        // 글쓰기, 수정하기, 거의 비슷, 수정하기 코드 복붙해서 sql 수정해서 사용하기.
        // sql 문장 작성,
