...
public class _0204_6_TodoServiceTests {
    ...
    // 전역으로 선언만 하고, 실제 사용은 밑에서 초기화해서, 사용할 수 있는 형태로 만들기.
    private _0204_4_TodoService todoService;

    @BeforeEach
    public void ready() {
        // 여기서 전역으로 선언된 변수를 초기화해서 사용할 수 있게 만들기
        // 다른 메서드 안에서도 todoService를 이용할 수 있다.
        todoService = _0204_4_TodoService.INSTANCE;
    }

    @Test
    public void testRegister() throws Exception {
        // 1) DB 서버에 일을 시키는 클래스를 미리 만들었음.
        // 2) 서비스 클래스를 만들어서, 어떤 기능을 하는지를 명확하게 메모화.
        // 3) 서비스에서 기능들을 만들어서 1) 클래스에 연결시키기.
        // 준비물) 글쓰기 -> 글 제목, 날짜, 완료 여부
        // 화면에서 데이터를 받고서 -> TodoDTO에 담기. -> 해당 서비스에 전달
        _0204_1_TodoDTO todoDTO = _0204_1_TodoDTO.builder()
                .title("샘플제목, 오늘 점심 뭐먹지?")
                .dueDate(LocalDate.now())
                .finished(true)
                .build();

        log.info("로그 라이브러리 Log4j2 기능 테스트");
        log.info(todoDTO);
